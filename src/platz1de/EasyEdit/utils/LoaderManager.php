...
class LoaderManager
{
	 ...
	 * @param UpdateSubChunkBlocksPacket[] $preparedInjections
	 * @see          World::setChunk()
				...
				if ($loader instanceof Player) {
					if ($preparedInjections !== []) {
						foreach ($preparedInjections as $injection) {
							//Allows instant, flicker-free block setting, costly network wise
							$loader->getNetworkSession()->sendDataPacket($injection);
						}
					} else {
